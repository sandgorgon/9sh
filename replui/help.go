...
var bashZshHelp = []string{
	...
	"and a native program (9ed, ... -- native_programs) both get a",
	"namespace-only Path checked out and written back automatically",
	"instead of erroring -- a native program also doesn't need the %",
	"sigil at all, callable bareword like a builtin. Tab",
	"completion inside a bare Path offers both real filesystem and",
	"namespace entries, which is exactly how this tends to get typed.",
	"",
