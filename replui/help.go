...
var bashZshHelp = []string{
	...
	"-- native_programs) gets the Path passed through as literal path",
	"text, untouched, trusting the program to resolve it itself -- a",
	"native program also doesn't need the % sigil at all, callable",
	"bareword like a builtin. Tab completion inside a bare Path offers",
	"both real filesystem and namespace entries, which is exactly how",
	"this tends to get typed.",
	"",
