...
var builtinDocs = []BuiltinDoc{
	...
	{"cp", "cp(src, dst)",
		"Copies src's content to dst — both ordinary namespace Paths, so a real OS file, a remote /n/host mount, or anything else bound in can be either side. For a regular file, dst may be an existing file or a new one at an existing directory level. For a directory, dst is created fresh as a full recursive copy — dst must not already exist."},
	{"rm", "rm(path)",
...
var namespaceAppNames = map[string]bool{
	...
	"vars": true, "unset": true, "ps": true, "binds": true, "wait": true,
