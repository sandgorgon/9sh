...
//
// Populated in init(), not a plain var initializer: a direct
// `var builtins = map[...]{"where": biWhere, ...}` creates a real Go
// initialization-order cycle, because biWhere calls closures via
// call/callClosure, which (through the ordinary tree-walking
// evaluator) can reach evalBackgroundInproc, which calls NewGlobalEnv,
// which reads builtins right back -- a self-reference the compiler
// rejects even though nothing is actually called during init (biWhere
// itself never runs until some kyu code pipes into `where`, long after
// package init finishes). init() sidesteps this: builtins' own
// declaration has no initializer to analyze for cycles, and assignments
// inside any func body (init included) are free to reference anything,
// resolved at call time rather than at initialization-order-graph
// construction time.
