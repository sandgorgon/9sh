...
var builtinDocs = []BuiltinDoc{
	...
	{"native_programs", "cmdname arg1 arg2 ...  (no % needed)", `A third call form, for external programs that are themselves namespace-aware (e.g. 9ed) rather than legacy Bytes-only binaries: any name listed in the native_programs kyu variable (see /config/config.ky) can be called bareword, no % sigil, same argument shape %cmd takes. A Path argument is passed through as its literal path text, untouched — no checkout, no scratch copy — trusting the program to resolve it itself (dial 9sh's namespace socket, Walk an absolute path or one rooted at /local, fall back to a real OS path only if the namespace doesn't claim it). Extend it like fullscreen_programs: native_programs := native_programs + ["mytool"].`},
