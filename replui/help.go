...
var keybindingHelp = []string{
	...
	"                         history_clear() clears that separately)",
	...
	"Ctrl+C                   copy the whole transcript -- or, while a",
...
var startupSequenceHelp = []string{
	...
	"   defaults (fullscreen_programs, native_programs, history_mode) the",
	"   first time only; an existing file is never overwritten.",
	"6. /ns bound -- a read-only view of this namespace's own binds",
	"   (/ns/binds, /ns/binds.json, /ns/log, /ns/log.json); see binds()",
	"   and bind_log().",
	"7. /session bound -- best-effort (needs 9vcs on PATH and a home",
	"   directory); the directory is bound either way, so past history",
	"   stays readable even when the recorder itself couldn't start.",
	"8. The shared kyu Env is created, with all of the above already",
	"   live in the namespace.",
	"9. config.ky runs (~/.config/9/config/config.ky) -- settings like",
	...
	"",
	"config.ky (9) runs before dotfiles (10) on purpose, so a dotfile can",
	...
	"Nothing in steps 3-10 is fatal to starting the shell except",
	...
	"each other.",
	"",
	"Steps 9-10 are also callable again at runtime, without restarting 9sh:",
